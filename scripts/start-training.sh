...
for  i in $(seq 0 $NUM_PS); do
  ...
  gcloud beta compute scp --recurse \
    /tmp/tf_config.json start-dist.sh ../trainer/ \
    ps-${i}:$WORKDIR
  gcloud compute ssh ps-${i} -- $WORKDIR/start-dist.sh &
done
...
for  i in $(seq 0 $NUM_WORKER); do
  ...
  gcloud beta compute scp --recurse \
    /tmp/tf_config.json start-dist.sh ../trainer/ \
    worker-${i}:$WORKDIR
  gcloud compute ssh worker-${i} -- $WORKDIR/start-dist-.sh &
done
...
gcloud beta compute scp --recurse \
  ...
  master-0:$WORKDIR
gcloud compute ssh master-0 -- $WORKDIR/start-dist.sh
