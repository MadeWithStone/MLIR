...
class PredictionModel:
    ...
    def predict(self, input_to_predict: Union[np.ndarray, str]) -> dict:
        ...
        ret = self.session.run(output, feed_dict={input_tensor: input_to_predict})
        current_acc = self.session.run(output)
...
class PredictionModelBatch:
    ...
    def __init__(self, model_dir: str, session: tf.Session = None, signature: str=DEFAULT_SERVING_SIGNATURE_DEF_KEY):
        ...
        self._init_op = self.g.get_operation_by_name('dataset_init')
    ...
    def predict(self, input_to_predict: List[str], batch_size: int=128) -> dict:
        ...
        current_acc = self.session.run([self._init_op, self._output_dict])
...
model = tf.saved_model.loader.load(self.session, [tag_constants.SERVING], model_dir)
...
if signature == DEFAULT_SERVING_SIGNATURE_DEF_KEY:
    ...
    raise NotImplementedError
        ...
        # Get init op for dataset
g = tf.get_default_graph()
_init_op = self.g.get_operation_by_name('dataset_init')
print(g)